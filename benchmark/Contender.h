...
class Contender {
  public:
	...
	virtual void beforeConstruction(const std::vector<std::string>& keys) { (void)keys; }
	virtual void beforeQueries(const std::span<std::string> keys) { (void)keys; }
		...
		if(numQueries > 0) {
			...
			if(numQueryThreads == 1) {
				...
				for(size_t i = 0; i < numQueryThreads; i++) {
					...
					threads.emplace_back([&querySpan, this] { performQueries(querySpan); });
				}
				for(size_t i = 0; i < numQueryThreads; i++) {
					threads.at(i).join();
				}
	...
	void printResult(std::string additional = "") {
		...
		std::cout << "RESULT"
		          << " name=" << name() << " bitsPerElement=" << bitsPerElement
		          << " constructionTimeMilliseconds="
		          << (constructionTimeMicroseconds < 10000
		          ...
		          << " numQueriesTotal=" << (numQueries * numQueryThreads) << " N=" << N
		          << " loadFactor=" << loadFactor << " threads=" << numThreads
		          << " queryThreads=" << numQueryThreads << additional << std::endl;
	}
  ...
  protected:
	...
	void doPerformTest(const std::span<std::string> keys, F& hashFunction) {
		...
		for(size_t i = 0; i < keys.size(); i++) {
			...
			if(taken[retrieved]) {
				std::cout << "Error: Collision: Key #" << i << "/" << N << " resulted in "
				          << retrieved << ": " << keys[i] << std::endl;
